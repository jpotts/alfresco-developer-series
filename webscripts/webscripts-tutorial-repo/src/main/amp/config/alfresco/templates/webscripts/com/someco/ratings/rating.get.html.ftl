		...
		<script type="text/javascript">
			...
			function postRating(id, rating, user) {
		        if (receiveReq.readyState == 4 || receiveReq.readyState == 0) {
		                receiveReq.open("POST", "/alfresco/service/someco/rating?id=" + id + "&rating=" + rating + "&guest=true&user=" + user, true);
			...
			function deleteRatings(id) {
		        if (receiveReq.readyState == 4 || receiveReq.readyState == 0) {
		                receiveReq.open("DELETE", "/alfresco/service/someco/rating/delete.html?id=" + id);
